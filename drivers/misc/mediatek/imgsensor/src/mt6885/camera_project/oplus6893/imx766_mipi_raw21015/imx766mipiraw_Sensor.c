...
static void read_module_data(void)
{
    ...
    gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN][0] = Eeprom_1ByteDataRead(0x00, IMX766_EEPROM_SLAVE_ADDRESS);
    gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN][1] = Eeprom_1ByteDataRead(0x01, IMX766_EEPROM_SLAVE_ADDRESS);
    imgsensor_info.module_id = Eeprom_1ByteDataRead(0x00, IMX766_EEPROM_SLAVE_ADDRESS);
    Oplusimgsensor_Registdeviceinfo(gImgEepromInfo_21015.pCamModuleInfo[IMGSENSOR_SENSOR_IDX_MAIN].name,
        gImgEepromInfo_21015.pCamModuleInfo[IMGSENSOR_SENSOR_IDX_MAIN].version, imgsensor_info.module_id);
    /*Read SensorId LensId and VCMId: 2~7*/
    for(i = 2; i < 8; i++) {
        gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN][i] = Eeprom_1ByteDataRead(0x04+i, IMX766_EEPROM_SLAVE_ADDRESS);
    }
    /*Read QR Code from eeprom: 8~27*/
    for (i = 0; i < OPLUS_CAMERASN_LENS; i ++) {
        gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN][8+i] = Eeprom_1ByteDataRead(0xB0+i, IMX766_EEPROM_SLAVE_ADDRESS);
    }
    ...
    memcpy(&gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN2][28], &Dac_master, 4); //set AFCodeData to MAIN2.
    memcpy(&gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN][40], &vcmID, 4);
    memcpy(&gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN][44], &lensID, 4);
    memcpy(&gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN][48], &Dac_mac, 4);
    memcpy(&gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN][52], &Dac_inf, 4);
    /*Read stereo eeprom data*/
    for (i = 0; i < CALI_DATA_MASTER_LENGTH; i ++) {
        gImgEepromInfo_21015.stereoMWdata[i] = Eeprom_1ByteDataRead(IMX766_STEREO_START_ADDR+i, IMX766_EEPROM_SLAVE_ADDRESS);
    }
    gImgEepromInfo_21015.i4CurSensorIdx = IMGSENSOR_SENSOR_IDX_MAIN;
    gImgEepromInfo_21015.i4CurSensorId = imgsensor_info.sensor_id;
}
