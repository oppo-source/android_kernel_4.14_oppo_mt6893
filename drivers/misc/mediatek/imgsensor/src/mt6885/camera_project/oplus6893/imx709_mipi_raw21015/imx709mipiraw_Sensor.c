...
static void read_module_data(void){
    ...
    gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_SUB][0] = Eeprom_1ByteDataRead(0x00, IMX709_EEPROM_SLAVE_ADDRESS);
    gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_SUB][1] = Eeprom_1ByteDataRead(0x01, IMX709_EEPROM_SLAVE_ADDRESS);
    imgsensor_info.module_id = Eeprom_1ByteDataRead(0x00, IMX709_EEPROM_SLAVE_ADDRESS);
    Oplusimgsensor_Registdeviceinfo(gImgEepromInfo_21015.pCamModuleInfo[IMGSENSOR_SENSOR_IDX_SUB].name,
        gImgEepromInfo_21015.pCamModuleInfo[IMGSENSOR_SENSOR_IDX_SUB].version, imgsensor_info.module_id);
    /*Read SensorId LensId and VCMId: 2~7*/
    for(i = 2; i < 8; i++) {
        gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_SUB][i] = Eeprom_1ByteDataRead(0x04+i, IMX709_EEPROM_SLAVE_ADDRESS);
    }
    /*Read QR Code from eeprom: 8~27*/
    for (i = 0; i < OPLUS_CAMERASN_LENS; i ++) {
        gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_SUB][8+i] = Eeprom_1ByteDataRead(0xB0+i, IMX709_EEPROM_SLAVE_ADDRESS);
    }
    ...
    memcpy(&gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_SUB][40], &vcmID, 4);
    memcpy(&gImgEepromInfo_21015.camNormdata[IMGSENSOR_SENSOR_IDX_SUB][44], &lensID, 4);
    gImgEepromInfo_21015.i4CurSensorIdx = IMGSENSOR_SENSOR_IDX_SUB;
