...
static int aw86907_ram_update(struct aw86907 *aw86907)
{
    ...
    if(aw86907->device_id == 815) {
        if (aw86907->f0 < OPPO_F0_VAL_MIN_0815 || aw86907->f0 > OPPO_F0_VAL_MAX_0815) {
            aw86907->f0 = 1700;
        }
    } else if(aw86907->device_id == 832) {
        if (aw86907->f0 < OPPO_F0_VAL_MIN_0832 || aw86907->f0 > OPPO_F0_VAL_MAX_0832) {
         aw86907->f0 = 2350;
        }
    } else {
        if (aw86907->f0 < OPPO_F0_VAL_MIN_0833 || aw86907->f0 > OPPO_F0_VAL_MAX_0833) {
           aw86907->f0 = 2350;
        }
    }
...
const struct firmware *aw86907_old_work_file_load_accord_f0(struct aw86907 *aw86907)
{
    ...
    if (aw86907->rtp_file_num == AW86907_OPPO_WAVEFORM_INDEX_OLD_STEADY
         || aw86907->rtp_file_num == AW86907_OPPO_WAVEFORM_INDEX_HIGH_TEMP)
    {
        if(aw86907->device_id == 815){
            if(aw86907->f0 <= 1610)
                f0_file_num = 0;
...
const struct firmware *aw86907_rtp_load_file_accord_f0(struct aw86907 *aw86907)
{
    ...
    else if (aw86907->rtp_file_num == AW86907_OPPO_RTP_LONG_SOUND_INDEX
        || aw86907->rtp_file_num == AW86907_OPPO_WAVEFORM_INDEX_OLD_STEADY)
    {
        if (aw86907->f0 <= 1650)
            f0_file_num = 0;
        else if (aw86907->f0 <= 1670)
        ...
        if (aw86907->rtp_file_num == AW86907_OPPO_RTP_LONG_SOUND_INDEX) {
            ret = request_firmware(&rtp_file,
                    aw86907_long_sound_rtp_name[f0_file_num],
                    aw86907->dev);
        }
        else if (aw86907->rtp_file_num == AW86907_OPPO_WAVEFORM_INDEX_OLD_STEADY){
            ret = request_firmware(&rtp_file,
                    aw86907_old_steady_test_rtp_name_0815[f0_file_num],
                    aw86907->dev);
        }
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (aw86907->state)
    {
        ...
        if (aw86907->device_id == 815)
            aw86907_haptic_set_gain(aw86907, aw86907->gain);
        aw86907_haptic_set_repeat_wav_seq(aw86907, AW86907_OPPO_WAVEFORM_INDEX_SINE_CYCLE);
...
static void oppo_motor_old_test_work(struct work_struct *work)
{
    ...
    if (aw86907->motor_old_test_mode == MOTOR_OLD_TEST_TRANSIENT) {
        ...
        aw86907_haptic_set_wav_seq(aw86907, 0, AW86907_OPPO_WAVEFORM_INDEX_TRANSIENT);
        ...
        aw86907_interrupt_clear(aw86907);
        mutex_unlock(&aw86907->lock);
        if(AW86907_OPPO_WAVEFORM_INDEX_OLD_STEADY < (sizeof(aw86907_rtp_name)/AW86907_RTP_NAME_MAX)) {
            aw86907->rtp_file_num = AW86907_OPPO_WAVEFORM_INDEX_OLD_STEADY;
            if(AW86907_OPPO_WAVEFORM_INDEX_OLD_STEADY) {
                //schedule_work(&aw86907->rtp_work);
                queue_work(system_unbound_wq, &aw86907->rtp_work);
            }
        } else {
        ...
        aw86907_interrupt_clear(aw86907);
        mutex_unlock(&aw86907->lock);
        if(AW86907_OPPO_WAVEFORM_INDEX_HIGH_TEMP < (sizeof(aw86907_rtp_name)/AW86907_RTP_NAME_MAX)) {
            aw86907->rtp_file_num = AW86907_OPPO_WAVEFORM_INDEX_HIGH_TEMP;
            if(AW86907_OPPO_WAVEFORM_INDEX_HIGH_TEMP) {
                //schedule_work(&aw86907->rtp_work);
                queue_work(system_unbound_wq, &aw86907->rtp_work);
            }
        } else {
        ...
        aw86907_interrupt_clear(aw86907);
        mutex_unlock(&aw86907->lock);
        if(AW86907_OPPO_WAVEFORM_INDEX_LISTEN_POP < (sizeof(aw86907_rtp_name)/AW86907_RTP_NAME_MAX)) {
            aw86907->rtp_file_num = AW86907_OPPO_WAVEFORM_INDEX_LISTEN_POP;
            if(AW86907_OPPO_WAVEFORM_INDEX_LISTEN_POP) {
                //schedule_work(&aw86907->rtp_work);
                queue_work(system_unbound_wq, &aw86907->rtp_work);
            }
        } else {
