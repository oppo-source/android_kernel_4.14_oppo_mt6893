...
struct IMGSENSOR_SENSOR_LIST *Oplusimgsensor_Sensorlist(void)
{
    ...
    if (is_project(21015) || is_project(21217)) {
        pOplusImglist =  gimgsensor_sensor_list_21015;
        pr_debug("gimgsensor_sensor_list_21015 Selected\n");
...
struct IMGSENSOR_HW_CFG *Oplusimgsensor_Custom_Config(void)
{
    ...
    if (is_project(21015) || is_project(21217)) {
        pOplusImgHWCfg = imgsensor_custom_config_21015;
        pr_debug("imgsensor_custom_config_21015 Selected\n");
        ...
        enum  IMGSENSOR_POWER_ACTION_INDEX  pwr_actidx)
{
    ...
    if (is_project(21015) || is_project(21217)) {
        if (pwr_actidx == IMGSENSOR_POWER_MATCHSENSOR_HWCFG_INDEX) {
            ppwr_seq = sensor_power_sequence_21015;
            pr_debug("[%s] match sensor_power_sequence_21015\n", __func__);
        } else {
            pr_debug("[%s] 21015 NOT Support MIPISWITCH\n", __func__);
        }
        return ppwr_seq;
    }
    if (is_project(20615)) {
        ...
        enum   IMGSENSOR_HW_POWER_STATUS pwr_status)
{
    ...
    enum IMGSENSOR_HW_ID hw_id = psensor_pwr->id[pin];

    pr_debug("[%s] pmic_ldo_get_type:%d pwr_status:%d avddIdx:%d, Hw id of the pin:%d is:%d",
                    __func__, pmic_ldo_get_type(), pwr_status, avddIdx, hw_id, pin);
    if(hw_id != IMGSENSOR_HW_ID_REGULATOR || pin == IMGSENSOR_HW_PIN_DOVDD){
        //Oplusimgsensor_ldo_powerset just process the fan53870 LDO
        return IMGSENSOR_RETURN_ERROR;
    }
    if(is_project(21015) || is_project(21217)){
