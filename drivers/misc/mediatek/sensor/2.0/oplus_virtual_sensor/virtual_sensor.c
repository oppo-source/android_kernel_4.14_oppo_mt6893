...
void oplus_init_sensor_state(struct SensorState *mSensorState)
{
    ...
    mSensorState[SENSOR_TYPE_FACE_DOWN_DETECT].sensorType = SENSOR_TYPE_FACE_DOWN_DETECT;
    mSensorState[SENSOR_TYPE_FACE_DOWN_DETECT].rate = SENSOR_RATE_ONCHANGE;
    ...
    p = &mSensorState[SENSOR_TYPE_FACE_DOWN_DETECT];
    p->gain = 1;
    strlcpy(p->name, "face_down", sizeof(p->name));
    strlcpy(p->vendor, "oplus", sizeof(p->vendor));
...
static int handle_to_index(int handle)
{
    ...
    case ID_SENSOR_MONITOR:
        ...
        break;
    case ID_FACE_DOWN_DETECT:
        index = face_down_detect;
        break;
    default:
        index = -1;
