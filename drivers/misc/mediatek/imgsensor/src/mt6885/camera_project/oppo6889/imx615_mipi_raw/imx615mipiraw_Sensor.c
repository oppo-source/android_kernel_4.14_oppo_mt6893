...
static void read_EepromQSC(void)
{
    ...
    gImgEepromInfo.camNormdata[1][48] = 0xFF;
    gImgEepromInfo.camNormdata[1][49] = 0xFF;
    ...
    for(i = 2; i < 8; i++) {
        ...
        gImgEepromInfo.camNormdata[1][48+i] = 0xFF;
    }
    for (i = 0; i < OPLUS_CAMERASN_LENS; i ++) {
        gImgEepromInfo.camNormdata[1][8+i] = Eeprom_1ByteDataRead(0xB0+i, 0xA8);
    }
    vcmID = (Eeprom_1ByteDataRead(0xb, 0xA8) << 8) | Eeprom_1ByteDataRead(0xa, 0xA8);
    lensID = (Eeprom_1ByteDataRead(0x9, 0xA8) << 8) | Eeprom_1ByteDataRead(0x8, 0xA8);
    memcpy(&gImgEepromInfo.camNormdata[1][40], &vcmID, 4);
    memcpy(&gImgEepromInfo.camNormdata[1][44], &lensID, 4);
