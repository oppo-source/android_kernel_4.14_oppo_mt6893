	...
	while (ppwr_info->pin != IMGSENSOR_HW_PIN_NONE &&
		...
		if (pwr_status == IMGSENSOR_HW_POWER_STATUS_ON) {
			if (ppwr_info->pin != IMGSENSOR_HW_PIN_UNDEF) {
				#ifdef VENDOR_EDIT
				if (Oplusimgsensor_ldo_powerset(phw, sensor_idx, ppwr_info->pin, pwr_status)
		...
		enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	...
	#ifdef VENDOR_EDIT
	/*Added by rentianzhi  2021/06/01, Bringup camera for 21015.*/
	if(strstr(phw->enable_sensor_by_index[(uint32_t)sensor_idx], "imx709_mipi_mono21015")){
		for(index = 0; index < IMGSENSOR_SENSOR_IDX_MAX_NUM; ++index ){
			if(strstr(phw->enable_sensor_by_index[(uint32_t)index], "imx709_mipi_raw21015")){
				sensor_idx = index;
				break;
			}
		}
